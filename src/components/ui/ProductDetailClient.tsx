  ...
  return (
    <MobileShell showHeader={false}>
      ...
      <div className="w-full aspect-square rounded-2xl border border-zinc-900 bg-[#07070a] relative overflow-hidden flex items-center justify-center mb-3">
        ...
        {currentImageUrl.startsWith('http') || currentImageUrl.startsWith('/') ? (
          <Image
            src={currentImageUrl}
            alt={product.name}
            fill
          ...
          <img src={currentImageUrl} alt={product.name} className="w-full h-full object-cover transition-all duration-300" />
        )}
        ...
        <div className="flex gap-2.5 overflow-x-auto pb-2 mb-4 scrollbar-thin scrollbar-thumb-zinc-800 scrollbar-track-transparent">
          {images.map((img, idx) => (
            <button
              key={idx}
              onClick={() => setActiveImageIndex(idx)}
              className={`w-14 h-14 relative rounded-xl overflow-hidden border shrink-0 bg-[#07070a] transition-all duration-300 ${
                idx === activeImageIndex 
            ...
            >
              {img.image_url.startsWith('http') || img.image_url.startsWith('/') ? (
                ...
                <img src={img.image_url} alt="mini" className="w-full h-full object-cover" />
              )}
            </button>
          ))}
        </div>
      ...
      <div className="flex flex-col mb-6">
        ...
        <div className="grid grid-cols-2 gap-3 mb-6">
          ...
          <div className="p-3 bg-[#0c0c0f] border border-zinc-900 rounded-lg">
            ...
            <span className="text-[10px] font-cyber-inter font-semibold text-zinc-300">
              {product.short_description.split(',')[1] || 'Variável'}
            </span>
          </div>
        </div>
        ...
        <p className="font-cyber-inter text-[11px] text-zinc-400 leading-relaxed mb-6 whitespace-pre-line">
          {product.description}
        </p>
